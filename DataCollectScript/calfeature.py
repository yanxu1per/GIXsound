...
for i in range(0,360,2):
    ...
    for file in onlyfile:
        ...
        features1 = predict.time_difference_mics(file)
        # we only want abs(features) <= 11, cuz only that make sense, the largest time can possibly get
        if max(features1)<=11 and min(features1)>=-11:
            features1 = [float(x)/11 for x in features1]
            features2 = predict.max_energy_mics(file)
            features2 = [float(x) for x in features2]
            features1.extend(features2)
            train_val.append(features1)
